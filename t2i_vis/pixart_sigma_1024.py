from diffusers import DPMSolverMultistepScheduler, StableDiffusionPipeline, UniPCMultistepScheduler
import torch
...
model_path = "/data/songtianhui.sth/models/PixArt-XL-2-1024-MS/"
...
prompts_list = prompts_list[101:250]
for prompts in prompts_list:
    ...
    for step in range(5, 11):
        ...
        dpmimage: Image = pipe(prompt, num_inference_steps=step, generator=torch.Generator(0), guidance_scale=2).images[
            0]
        pipe.scheduler = unipcsolver
        unipcimage: Image = \
        pipe(prompt, num_inference_steps=step, generator=torch.Generator(0), guidance_scale=2).images[0]
        pipe.scheduler = NeuralSolver(num_train_timesteps=1000)
        dssimage: Image = pipe(prompt, num_inference_steps=step, generator=torch.Generator(0), guidance_scale=2).images[
            0]
    ...
    vertical_concat = Image.new("RGB", (dpm_horizontal_concat.width,
                                        dpm_horizontal_concat.height + unipc_horizontal_concat.height + dss_horizontal_concat.height))
    ...
    # vertical_concat.save(f"./pixart-sigma-1024/{remove_invalid_chars(prompt)}.png")
    vertical_concat.save(f"./pixart-sigma-1024/{remove_invalid_chars(prompt)}.jpg")
